...
export class Manager extends EventEmitter {
  ...
  private async onReady (contactOrRoomList: Contact[]) {
    ...
    logger.info(
      `onReady() all contacts and rooms are ready, friendCount: ${friendCount} contactCount: ${contactCount} roomCount: ${roomCount}`,
    )
    this.emit('ready')
  }
  ...
  private async onMessage (message: Message) {
    ...
    // @ts-ignore
    if (message.type === 'multi_vcard' || (message.type === 'e2e_notification' && message.body === '' && !message.author)) {
  ...
  public async initWhatsAppEvents (whatsapp: WhatsApp): Promise<void> {
    ...
    const events = ['authenticated', 'ready', 'disconnected']
